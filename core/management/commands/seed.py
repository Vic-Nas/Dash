...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username='Vic-Nas').exists():
            ...
            profile.objects.create(user=admin, coins=5000)
        ...
        admin_user = User.objects.get(username='Vic-Nas')
        default_skin = botSkin.objects.get(name='Classic Blue')
        ownedSkin.objects.get_or_create(player=admin_user, skin=default_skin)
